...
Private Declare Function SHBrowseForFolder Lib "shell32" Alias "SHBrowseForFolderA" (pBI As BROWSEINFO) As Long
...
Private Const BFFM_VALIDATEFAILED = 3&    ' If BIF_VALIDATE is specified in the ulFlags
...
Private Const BFFM_SETSTATUSTEXT = &H464& ' Display status text message.
Private Const BFFM_ENABLEOK = &H465&      ' Enable the OK button.
Private Const BFFM_SETSELECTION = &H466&  ' Selects the specified folder. The message's
...
' -------------------------------------------------------------
    ...
    If LenB(PromptMessage) Then
        sPromptMsg = StrConv(PromptMessage, vbFromUnicode)
    Else
        sPromptMsg = StrConv("Please select a folder...", vbFromUnicode)
    End If
    sDisplayName = FillNulls(MAX_PATH)
    If LenB(CheckboxMessage) Then mCheckboxTitle = "  " & CheckboxMessage
    If PathExists(StartingPath) Then mStartPath = AddSlash(StartingPath)
    eCheck = ShowCheckbox

    With tBrowseInfo
        .hwndOwner = Me_hWnd
        .pszDisplayName = StrPtr(sDisplayName)
        .lpszTitle = StrPtr(sPromptMsg)
    ...
    If (lpIDList <> 0&) Then
        ...
        If (SHGetPathFromIDList(lpIDList, sBuffer) <> 0&) Then
  ...
  'obtain the pidl to the special folder 'network'
   If SHGetSpecialFolderLocation(Me_hWnd, CSIDL_NETWORK, pidlNet) = ERROR_SUCCESS Then
      ...
      If LenB(PromptMessage) Then
          sPromptMsg = StrConv(PromptMessage, vbFromUnicode)
      Else
          sPromptMsg = StrConv("Please select a computer...", vbFromUnicode)
      End If
      sDisplayName = FillNulls(MAX_PATH)
      If LenB(CheckboxMessage) Then mCheckboxTitle = "  " & CheckboxMessage
      eCheck = ShowCheckbox
     ...
     pidList = SHBrowseForFolder(tBrowseInfo)
     ...
     If (pidList <> 0&) Then
         ...
         'which we return
          BrowseNetwork = TrimNulls(StrConv(sDisplayName, vbUnicode)) ' tBrowseInfo.pszDisplayName = StrPtr(sDisplayName)
...
Private Function BrowseCallback(ByVal hWnd As Long, ByVal uMsg As Long, ByVal lParam As Long, ByVal pData As Long) As Long
    ...
    If (uMsg = BFFM_SELCHANGED) Then
        Dim sBuffer As String, sStatus As String
        sBuffer = FillNulls(MAX_PATH)
        If (SHGetPathFromIDList(lParam, sBuffer) <> 0&) Then
            sStatus = TrimNulls(sBuffer)
            ' Set the status area to the currently selected path
            SendMessageString hWnd, BFFM_SETSTATUSTEXT, 0&, sStatus
        End If
    ElseIf (uMsg = BFFM_INITIALIZED) Then
        CentreBrowsePos hWnd
        If (LenB(mStartPath) <> 0&) Then
            ' wParam is True (1) since you are passing a string path.
            ' It would be False (0) if you were passing a pIDL
            SendMessageString hWnd, BFFM_SETSELECTION, 1&, mStartPath
        End If
        If eCheck <> DontShow Then
            ' Thanks to Bobo Enterprises for checkbox code :)
            GetWindowRect hWnd, tRect
            hWndCheckbox = CreateWindowEx(WS_EX_TRANSPARENT, "Button", mCheckboxTitle, WS_CHILD Or BS_CHECKBOX, 25&, tRect.Bottom - tRect.Top - 60&, 400&, 25&, hWnd, 0&, App.hInstance, tCS)
            hFont = SendMessageLong(hWnd, WM_GETFONT, 0&, 0&)
            SendMessageLong hWndCheckbox, WM_SETFONT, hFont, 1&
            SendMessageLong hWndCheckbox, BM_SETCHECK, eCheck, 1&
...
Private Function CheckboxCallback(ByVal hWnd As Long, ByVal uMsg As Long, ByVal wParam As Long, ByVal lParam As Long) As Long
    ...
    If uMsg = WM_LBUTTONDOWN Then
        eCheck = Abs(SendMessageLong(hWndCheckbox, BM_GETCHECK, 0&, 0&) - 1&)
        SendMessageLong hWndCheckbox, BM_SETCHECK, eCheck, 1&
    Else
...
Public Function PathExists(sPath As String) As Boolean
    PathExists = Not (GetAttributes(sPath) = -1&)
End Function
